...
int main(void) {
     ...
     * string有多种初始化方式:
     * 1. 默认为 empty string
     * 2. 使用 = operator 从 string literal 复制(copy initialize)
     ...
     * > 然后再通过复制形式初始化拷贝了这个临时变量对象. (不推荐使用这种方式初始化)
     * 
     * **string 的有关操作:**
     * 
     * 1. 头文件 string 中有一个全局函数 istream& getline(istream&, string) 用于从 istream 中读取一行 string(回车结束, 
     * 结果不含有回车) 并且类 string 还重载了很多操作符, 其中 == 操作符判断两个 string 内容是否相同(区别大小写)
     * 
     ...
     * 
     * 2. string 的 assignment operator (= 号赋值操作) 区别于初始化的 = sign, assignment operator 只是将
     * string 变量中的内容替换掉. string 还重载了 + 号操作符用于连接(append/concatenation)两个字符串并
     * 返回一个新的 string 对象. operator + 的右操作符有很多重载类型.
     * 
     * 3. string 成员函数 bool empty(), 
     * 
     * 头文件 cctype 中有很多判断字符的函数.
     * 
     * 4. string 成员函数 size() 返回 string::size_type, 和其他 STL 一样中的类一样, 为了使得库中的类型更好的使用
     * machine-dependent manner (也就是为了移植性), 很多健壮的库都采用这种方式. 这会给类库消费者来带一些冗杂的感觉,
     * 不过通过使用C++11的 auto 可以让关键字自动得帮我们推导.
     * 
     * 5. string 同样重载了 operator [], 不过string的 [] 越界访问的行为是 **未定义** 的(UB).
     * 
     * 6. string 重载了 operatot+(), 但是两个 C 风格字符串( string literal 是用 const char array 表示的)
     * 不能用 + 操作符. 
     * 
     ...
     * > 采用 name.h 的C语言库.
     * 
     * C++ 11 新增 range-base for (for-each) 来更加好的遍历访问.
    ...
    // string s1 = "hello" + "world"; // 两个 C 风格字符串之间不能用 + 操作符.
    
    // Exercise 3.9
    // string 的 subscript 是不检查越界的, 虽然下面代码非法, 但是我可以编译通过.
    string s2;
    cout << s2[0] << endl;
